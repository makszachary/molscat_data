...
pickle_dir_path = Path(__file__).parents[1] / 'data_produced' / 'pickles'
...
def save_and_plot_k_L_E_spinspin(pickle_path: Path | str, singlet_phase: float, triplet_phase: float, so_scaling: float):
    ...
    phase = (singlet_phase, triplet_phase)
    param_values = {'singletParameter': (default_singlet_parameter_from_phase(singlet_phase),), 'tripletParameter': (default_triplet_parameter_from_phase(triplet_phase),)}
    spin_orbit_scaling = s_matrix_collection.spinOrbitParameter[0]
    k_L_E_arrays = tuple([rate_fmfsms_vs_L_multiprocessing(s_matrix_collection, 4, MF_out, 1, -1, 4, 4, 1, 1, param_values = param_values, unit = 'cm**3/s') for MF_out in range(-4, 4+1, 2) ] )
...
def plot_k_L_E(energy_array, k_L_E_array, exp_probability = 0.1790, exp_probability_std = 0.0100, transfer_rate = 2.71e-9):
    total_k_L_E_array = k_L_E_array.sum(axis=0).squeeze() 
    fig, ax = PartialRateVsEnergy.plotRate(energy_array, k_L_E_array)
    ax.plot(energy_array, total_k_L_E_array, label = "total", linewidth = 3, linestyle = 'solid', color = 'k')
    ax.axhline(exp_probability, color = 'k', linewidth = 1, linestyle = '--', label = 'experimental value')
    ax.axhspan(exp_probability-exp_probability_std, exp_probability+exp_probability_std, color='0.5', alpha=0.5)
