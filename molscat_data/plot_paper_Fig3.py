...
def plotFig3(singlet_phases: float | np.ndarray[float], triplet_phases: float | np.ndarray[float], phase_differences: float | np.ndarray[float], phase_difference_distinguished: float, so_scaling: float, magnetic_phases: tuple[tuple[float, float], ...], magnetic_fields: float | np.ndarray[float], magnetic_field_experimental: float, MF_in: int, MS_in: int, energy_tuple: tuple[float, ...], temperatures: tuple[float, ...] = (5e-4,), plot_temperature: float = 5e-4, cm_input_dir_name: str = 'RbSr+_tcpld_80mK_0.01_step', vs_B_input_dir_name = 'RbSr+_fmf_vs_SE_80mK', colormap_hybrid = False, magnetic_enhanced = False, plot_section_lines = False, journal_name = 'NatCommun'):
    ...
    E_max = max(energy_tuple)
    singlet_phases, triplet_phases = np.array(singlet_phases), np.array(triplet_phases)
    probabilities_dir_name = 'probabilities_hybrid' if colormap_hybrid else 'probabilities'

    array_paths_cold_lower = [  [arrays_dir_path / cm_input_dir_name / f'{E_min:.2e}_{E_max:.2e}_{nenergies}_E' / f'{singlet_phase:.4f}_{triplet_phase:.4f}' / f'{so_scaling:.4f}' / probabilities_dir_name / 'cold_lower.txt' if triplet_phase % 1 != 0 else None for triplet_phase in triplet_phases] for singlet_phase in singlet_phases]
    [ [print(array_path) for array_path in sublist if not array_path.is_file()] for sublist in array_paths_cold_lower ]
    arrays_cold_lower = np.array([ [np.loadtxt(array_path) if (array_path is not None and array_path.is_file()) else np.full((len(temperatures), 3), np.nan) for array_path in sublist] for sublist in array_paths_cold_lower ])
    arrays_cold_lower = arrays_cold_lower.reshape(*arrays_cold_lower.shape[0:2], len(temperatures), -1)
    ...
    cm = 1/2.54
    ws, hs = 0.05, 0.05
    figsize = (18*cm, (1/(1-hs))*(7.2+6)*cm)
    dpi = 1000
    fig = plt.figure(figsize=figsize, dpi = dpi)
    gs_Fig3 = gridspec.GridSpec(int(1000*(1/(1-hs))*(7.2+6)),180, fig)
    # figs = fig.subfigures(2, 2, wspace = ws, hspace = hs)
    fig0 = fig.add_subfigure(gs_Fig3[int(1000*7.2/((1/(1-hs))*(7.2+6))),:90])
    fig1 = fig.add_subfigure(gs_Fig3[int(1000*7.2/((1/(1-hs))*(7.2+6))),90:])
    fig2 = fig.add_subfigure(gs_Fig3[int(1000*6/((1/(1-hs))*(7.2+6))),115:])
    fig3 = fig.add_subfigure(gs_Fig3[int(1000*6/((1/(1-hs))*(7.2+6))),115:])
    ...
    array_paths_cold_lower = [  [arrays_dir_path / cm_input_dir_name / f'{E_min:.2e}_{E_max:.2e}_{nenergies}_E' / f'{singlet_phase:.4f}_{(singlet_phase+phase_difference)%1:.4f}' / f'{so_scaling:.4f}' / probabilities_dir_name / 'cold_lower.txt' if ( singlet_phase+phase_difference ) % 1 !=0 else None for phase_difference in phase_differences] for singlet_phase in singlet_phases]
    [ [print(array_path) for array_path in sublist if (array_path is not None and not array_path.is_file())] for sublist in array_paths_cold_lower ]
    arrays_cold_lower = np.array([ [np.loadtxt(array_path) if (array_path is not None and array_path.is_file()) else np.full((len(temperatures), 3), np.nan) for array_path in sublist] for sublist in array_paths_cold_lower ])
    arrays_cold_lower = arrays_cold_lower.reshape(*arrays_cold_lower.shape[:2], len(temperatures), -1)

    if phase_difference_distinguished is not None:
        array_paths_cold_lower_distinguished = [arrays_dir_path / cm_input_dir_name / f'{E_min:.2e}_{E_max:.2e}_{nenergies}_E' / f'{singlet_phase:.4f}_{(singlet_phase+phase_difference_distinguished)%1:.4f}' / f'{so_scaling:.4f}' / probabilities_dir_name / 'cold_lower.txt' if ( singlet_phase + phase_difference_distinguished ) % 1 !=0 else None for singlet_phase in singlet_phases]
        arrays_cold_lower_distinguished = np.array([ np.loadtxt(array_path) if (array_path is not None and array_path.is_file()) else np.full((len(temperatures), 3), np.nan) for array_path in array_paths_cold_lower_distinguished ])
        arrays_cold_lower_distinguished = arrays_cold_lower_distinguished.reshape(arrays_cold_lower_distinguished.shape[0], len(temperatures), -1)
    ...
    ## (c) Spin-exchange probabilities vs the magnetic field
    probabilities_dir_name = 'probabilities'
    prefix_for_array_path = '' if magnetic_enhanced else 'p0_'
    F1, F2 = 2, 1
    MF1, MF2 = MF_in, MS_in

    abbreviation='cold'
    
    array_paths_cold_lower = [  [arrays_dir_path / vs_B_input_dir_name / f'{E_min:.2e}_{E_max:.2e}_{nenergies}_E' / f'{singlet_phase:.4f}_{triplet_phase:.4f}' / f'{F1}_{MF1}_{F2}_{MF2}' / f'{magnetic_field:.2f}' / probabilities_dir_name / f'{prefix_for_array_path}{abbreviation}.txt' for magnetic_field in magnetic_fields] for singlet_phase, triplet_phase in magnetic_phases]
    [ [print(array_path) for array_path in sublist if (array_path is not None and not array_path.is_file())] for sublist in array_paths_cold_lower ]
    arrays_cold_lower = np.array([ [np.loadtxt(array_path) if (array_path is not None and array_path.is_file()) else np.full((len(temperatures), 3), np.nan) for array_path in sublist] for sublist in array_paths_cold_lower ])
    arrays_cold_lower = arrays_cold_lower.reshape(*arrays_cold_lower.shape[:2], len(temperatures), -1)
    ...
    dpeff = 1e-3
    p0_std = (p0(peff_experiment+dpeff/2, pmf_array=pmf_array)-p0(peff_experiment-dpeff/2, pmf_array=pmf_array))/dpeff * peff_std_experiment
    experiment = peff_experiment if magnetic_enhanced else p0(peff_experiment, pmf_array)
    std = peff_std_experiment if magnetic_enhanced else p0_std
    ...
    props = dict(boxstyle='round', facecolor='none', edgecolor='midnightblue')
    fig2_ax.text(0.03, 0.10, f'$\\Delta\\Phi_\\mathrm{{fit}} = {(magnetic_phases[0][1]-magnetic_phases[0][0])%1:.2f}\\pi$', va = 'center', ha = 'left', transform = fig2_ax.transAxes, bbox = props)
    ylabel = f'$p_\mathrm{{eff}}$' if magnetic_enhanced else f'$p_0$'
    fig2_ax.set_ylabel(ylabel)
    ...
    fig0_ax.text(-0.18, 1.00, f'a', fontsize = 7, family = 'sans-serif', va = 'top', ha = 'left', transform = fig0_ax.transAxes, fontweight = 'bold')
    fig1_ax0.text(-0.18, 1.00, f'b', fontsize = 7, family = 'sans-serif', va = 'top', ha = 'left', transform = fig1_ax0.transAxes, fontweight = 'bold')
    fig2_ax.text(-0.09, 1.11, f'c', family = 'sans-serif', fontsize = 7, va = 'top', ha = 'left', transform = fig2_ax.transAxes, fontweight = 'bold')
    fig3_axs[0].text(-0.14, 1.33, f'd', family = 'sans-serif', fontsize = 7, va = 'top', ha = 'left', transform = fig3_axs[0].transAxes, fontweight = 'bold')
...
def AplotFig3(singlet_phases: float | np.ndarray[float], triplet_phases: float | np.ndarray[float], phase_differences: float | np.ndarray[float], phase_difference_distinguished: float, so_scaling: float, magnetic_phases: tuple[tuple[float, float], ...], magnetic_fields: float | np.ndarray[float], magnetic_field_experimental: float, MF_in: int, MS_in: int, energy_tuple: tuple[float, ...], temperatures: tuple[float, ...] = (5e-4,), plot_temperature: float = 5e-4, cm_input_dir_name: str = 'RbSr+_tcpld_80mK_0.01_step', vs_B_input_dir_name = 'RbSr+_fmf_vs_SE_80mK', colormap_hybrid = False, magnetic_enhanced = False, plot_section_lines = False, journal_name = 'NatCommun'):
    plt.style.use(Path(__file__).parent / 'mpl_style_sheets' / f'{journal_name}.mplstyle')
    nenergies = len(energy_tuple)
    E_min = min(energy_tuple)
    E_max = max(energy_tuple)
    ...
    fig0_ax.text(-0.18, 1.00, f'a', fontsize = 7, family = 'sans-serif', va = 'top', ha = 'left', transform = fig0_ax.transAxes, fontweight = 'bold')
    fig1_ax0.text(-0.18, 1.00, f'b', fontsize = 7, family = 'sans-serif', va = 'top', ha = 'left', transform = fig1_ax0.transAxes, fontweight = 'bold')
    fig2_ax.text(-0.09, 1.11, f'c', fontsize = 7, family = 'sans-serif', va = 'top', ha = 'left', transform = fig2_ax.transAxes, fontweight = 'bold')
    fig3_axs[0].text(-0.14, 1.33, f'd', fontsize = 7, family = 'sans-serif', va = 'top', ha = 'left', transform = fig3_axs[0].transAxes, fontweight = 'bold')
    fig.savefig(png_path, bbox_inches='tight', pad_inches = 0)
    fig.savefig(svg_path, bbox_inches='tight', pad_inches = 0, transparent = True)
    fig.savefig(pdf_path, bbox_inches='tight', pad_inches = 0, transparent = True)
    plt.close()
