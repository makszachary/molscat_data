...
def save_and_plot_average_vs_B(pickle_paths: tuple[Path, ...], MF_in: int = -2, MS_in: int = 1):
    ...
    ax.set_title(f'The rate of the spin-exchange for the $\\left|1,{int(MF_in/2)}\\right>\\hspace{{0.2}}\\left|\\hspace{{-.2}}{arrow}\\hspace{{-.2}}\\right>$ initial state. $(\\tilde{{\\Phi}}_\\mathrm{{s}}, \\tilde{{\\Phi}}_\\mathrm{{t}}) = ({phases[0][0]:.2f}\\pi, {phases[0][1]:.2f}\\pi).$')
    ...
    probabilities = np.array(averaged_rates) / 2.71e-9
    fig, ax  = RateVsMagneticField.plotRate(magnetic_fields, probabilities)
    ax.set_title(f'The short-range spin-exchange probability for the $\\left|1,{int(MF_in/2)}\\right>\\hspace{{0.2}}\\left|\\hspace{{-.2}}{arrow}\\hspace{{-.2}}\\right>$ initial state. $(\\tilde{{\\Phi}}_\\mathrm{{s}}, \\tilde{{\\Phi}}_\\mathrm{{t}}) = ({phases[0][0]:.2f}\\pi, {phases[0][1]:.2f}\\pi).$')
    ax.set_ylabel('probability $p_0$')
    ax.set_xlabel('magnetic field (G)')
    ax.minorticks_on()
    ax.tick_params(which = 'both', direction = 'in')
    
    ax.set_ylim(0, 1.25*max(probabilities))
    image_path = plots_dir_path / 'SE_vs_B_probabilities' / pickle_path.relative_to(pickle_dir_path).parent / f'{magnetic_fields[0]:.2f}_{magnetic_fields[-1]:.2f}_{(magnetic_fields[1]-magnetic_fields[0]):.2f}.png'
    image_path.parent.mkdir(parents=True, exist_ok=True)
    fig.savefig(image_path)
    plt.close()
...
def main():
    ...
    output_dirs = tuple( scratch_path / 'molscat' / 'outputs' / 'RbSr+_fmf_SE_vs_B' / f'{nenergies}_E' / f'2{args.MF_in}1{args.MS_in}' / f'{args.singlet_phase:.4f}_{args.triplet_phase:.4f}' / f'{magnetic_field}' for magnetic_field in magnetic_fields )
    #pickle_paths = []
    ...
    pickle_paths = tuple( pickle_dir_path / 'RbSr+_fmf_SE_vs_B' / f'{nenergies}_E' / f'2{args.MF_in}1{args.MS_in}' / f'{args.singlet_phase:.4f}_{args.triplet_phase:.4f}' / f'{magnetic_field}.pickle' for magnetic_field in magnetic_fields )
