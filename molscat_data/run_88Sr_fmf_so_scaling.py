...
def calculate_and_save_k_L_E_and_peff_parallel(pickle_path: Path | str, transfer_pickle_path: Path | str, F_in: int, MF_in: int, S_in: int, MS_in: int, phases = None, dLMax: int = 4, temperatures = (5e-4,)):
    ...
    shutil.unpack_archive(arrays_dir_path.joinpath(pickle_path.relative_to(pickles_dir_path)).parent.with_suffix('.zip'), arrays_dir_path.joinpath(pickle_path.relative_to(pickles_dir_path)).parent, 'zip')
...
def main():
    ...
    parser.add_argument("--logT_min", type = float, default = -3)
    parser.add_argument("--logT_max", type = float, default = -4)
