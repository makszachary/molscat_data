...
def create_and_run(molscat_input_template_path: Path | str, singlet_phase: float, triplet_phase: float, so_scaling: float, magnetic_field: float, F_in: int, MF_in: int, S_in: int, MS_in: int, energy_tuple: tuple[float, ...], L_max: int = 2*29, MTOT = None, spin_orbit_included = True) -> tuple[float, float, float]:
    ...
    if spin_orbit_included:
        ...
        scaled_so_path = scratch_path / 'molscat' / 'so_coupling' / molscat_input_template_path.parent.relative_to(molscat_input_templates_dir_path) / f'{E_min:.2e}_{E_max:.2e}_{nenergies}_E' / f'{singlet_phase:.4f}_{triplet_phase:.4f}' / molscat_input_template_path.stem / f'so_{so_scaling:.4f}_scaling_in_{F_in}_{MF_in}_{S_in}_{MS_in}_MTOT_{MTOT}.dat'
...
def calculate_and_save_k_L_E_and_peff_parallel(pickle_path: Path | str, transfer_pickle_path: Path | str, F_in: int, MF_in: int, S_in: int, MS_in: int, phases = None, dLMax: int = 4, temperatures = (5e-4,)):
    ...
    for abbreviation, name, arg in zip(*map(reversed, (abbreviations, names, args) ) ) :
        ...
        average_rate_arrays = np.array( [s_matrix_collection.thermalAverage(rate_array.sum(axis=len(arg[2].shape)), distribution_array) for distribution_array in distribution_arrays ] ).squeeze()
        ...
        print(f'{momentum_transfer_rate_array.sum(axis=len(arg[2].shape)).shape = }')
        print(f'{transfer_distribution_arrays[0].shape = }')
        average_momentum_transfer_arrays = np.array( [ transfer_s_matrix_collection.thermalAverage(momentum_transfer_rate_array.sum(axis=len(arg[2].shape)), transfer_distribution_array) for transfer_distribution_array in transfer_distribution_arrays ] ).squeeze()
