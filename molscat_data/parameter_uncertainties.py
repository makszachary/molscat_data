...
def main():
    ...
    if args.DPhi:
        ...
        popt, perr, chisq = fit_data(spin_exchange, phase_differences, theory_hot, bounds = ((-0.5, 0.4), (0.5, 0.7)))
        [Phi0_hot, amplitude_hot] = popt
        print(popt, perr, chisq)
        print(f"{Phi0_hot = }pi")
        popt, perr, chisq = fit_data(spin_exchange, phase_differences, theory_cold, bounds = ((-0.5, 0.1), (0.5, 0.4)))
        ...
        ax.plot(phase_differences, y2, color = 'midnightblue')

        ax.scatter(phase_differences, theory_hot, s = 8, facecolors='none', edgecolors = 'firebrick')#, marker = 'o')
        ...
        def numerical_sigmaDPhi(phase_difference_0, phase_differences, phase_step, theory, experiment, std):
            ...
            print(np.roll(theory, -1, axis = 1) - np.roll(theory, 1, axis = 1))
            print(np.roll(phase_differences, -1) - np.roll(phase_differences, 1))
            derivative =  (np.roll(theory, -1, axis = 1) - np.roll(theory, 1, axis = 1)) / (2*phase_step)
            return derivative

        drv = numerical_sigmaDPhi(0.2, phase_differences, 0.01, theory_distinguished, experiment, std)
