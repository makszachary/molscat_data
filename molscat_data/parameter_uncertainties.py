...
def main():
    ...
    if args.DPhi:
        ...
        brt = brute_fit(fun, phase_differences, np.full((*experiment.shape, *phase_differences.shape), experiment), np.full((*std.shape, *phase_differences.shape), std), bounds = ((0.1, 0.3),), Ns = 20)
        print(brt)
