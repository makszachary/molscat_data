...
def plotPeffAverageVsMassToFig(fig, singlet_phase: float, triplet_phase: float, so_scaling: float, reduced_masses: np.ndarray[float], energy_tuple_vs_mass_even: tuple[float, ...], energy_tuple_vs_mass_odd: tuple[float, ...], temperatures: np.ndarray[float] = np.array([5e-4,]), plot_temperature: float = 5e-4, even_input_dir_name: str = 'RbSr+_tcpld_80mK_vs_mass', odd_input_dir_name: str = 'RbSr+_tcpld_vs_mass_odd'):
    ...
    xx_text, yy_text = reduced_masses_experimental, experiment
    xx_text[[2,3]] -= 0.15
    yy_text[[0,1]] -= -0.05
    yy_text[[2,3]] += 0.04
    for x, y, label in zip(xx_text[[0,1,3]], yy_text[[0,1,3]], reduced_masses_labels[0:2]+[reduced_masses_labels[3],]):
        fig_ax.text(x, y, label, color = 'firebrick', fontsize = 'x-small', ha = 'center', va = 'top')
    fig_ax.text(xx_text[2], yy_text[2], reduced_masses_labels[2], color = 'darkmagenta', fontsize = 'x-small', ha = 'center', va = 'baseline')
