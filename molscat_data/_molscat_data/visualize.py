    ...
    def plotValues(cls, xx, theory, experiment, std, theory_distinguished = None, theory_colors = None, theory_distinguished_colors = None, figsize = (5.5, 1.5), dpi = 300):
        ...
        for i, yy in enumerate(np.moveaxis(theory, -1, 0)):
            ...
            try:
                ...
                ax.plot(xx[yy_mask].reshape(-1, xx.shape[-1]).squeeze(), yy[yy_mask].reshape(-1, yy.shape[-1]).flatten().squeeze(), color = theory_colors[i], linewidth = 3)
            except ValueError:
                print(f'{xx=}, {yy=}')
                ax.plot(xx, yy, color = theory_colors[i], linewidth = .4)
...
class ContourMap:
    ...
    def _initiate_plot(figsize = (5.5, 5), dpi=300):
        ...
        ax_bar = fig.add_subplot(1,110,(105,110))
