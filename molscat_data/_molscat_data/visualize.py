...
class ValuesVsModelParameters:
    ...
    def plotValuesAndChiSquaredToAxis(cls, ax, xx, theory, experiment = None, std = None, theory_distinguished = None, theory_formattings = None, theory_distinguished_formattings = None, experiment_formattings = None):
        ...
        ax.tick_params(which='both', direction='in', top = True, length = 3)
        ax.tick_params(which='minor', length = 1.5)

        ax_chisq.set_yscale('log')
        ax_chisq.tick_params(which='both', direction='in', length = 3)
        ax_chisq.tick_params(which='minor', length = 1.5)
...
class ContourMap:
    ...
    def plotToFigure(fig, ax, ax_bar, X, Y, FXY, n_levels = 11, cmap_name = 'cividis'):
        ...
        ax.clabel(con, inline=True, fontsize = mpl.rcParams["xtick.labelsize"])
