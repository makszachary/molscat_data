...
class ContourMap:
    ...
    def plotMap(cls, X, Y, FXY, n_levels = 11, cmap_name = 'cividis', figsize = (5.5, 4), dpi=300):
        ...
        con = ax.contour(X, Y, FXY.transpose(), levels = n_levels, colors='black', linestyles = '-', linewidths = 0.5,)
        # ax.clabel(con, inline=True, fontsize=8)
        im = ax.imshow(FXY.transpose(), cmap = plt.get_cmap(cmap_name), extent = (np.amin(X), np.amax(X), np.amin(Y), np.amax(Y)), origin='dotted')
        bar = fig.colorbar(im, orientation = 'vertical', cax = ax_bar)
