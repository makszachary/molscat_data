...
def rate_fmfsms_vs_L(s_matrix_collection: SMatrixCollection, F_out: int, MF_out: int, S_out: int, MS_out: int, F_in: int, MF_in: int, S_in: int, MS_in: int, param_indices: dict = None, dLMax: int = 4, unit = None) -> float:
    ...
    err_shapes = [ [ s_matrix_collection.getRateCoefficient(qn.LF1F2(L = L_out, ML = ML_in + 0 + 10 - (-2) - (-8), F1 = 2, MF1 = -2, F2 = 8, MF2 = -8), qn.LF1F2(L = L_in, ML = ML_in, F1 = 4, MF1 = 0, F2 = 10, MF2 = 10), unit = unit, param_indices = param_indices).shape for ML_in in range(-L_in, L_in+1, 2) for L_out in range(L_in - dLMax*2, L_in + dLMax*2+1, 2*2) if (L_out >= 0 and L_out <=L_max) and abs(ML_in + MF_in + MS_in - MF_out - MS_out) <= L_out ] for L_in in range(0, L_max+1, 2)]
    print(f'{err_shapes}')
    try:
