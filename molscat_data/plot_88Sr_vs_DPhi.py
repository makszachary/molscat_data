...
def plot_probability_vs_DPhi(singlet_phases: float | np.ndarray[float], phase_differences: float | np.ndarray[float], so_scaling: float, energy_tuple: tuple[float, ...], singlet_phase_distinguished: float = None, triplet_phases_distinguished: float = None, temperatures: tuple[float, ...] = (5e-4,), plot_temperature: float = 5e-4, input_dir_name: str = 'RbSr+_tcpld_80mK'):
    ...
    arrays_hot = np.array([ np.loadtxt(array_path) if array_path is not None else np.full(5, np.nan) for sublist in array_paths_hot for array_path in sublist ]).reshape(*(array_paths_hot.shape), len(temperatures), -1)
    arrays_cold_higher = np.array( [ [np.loadtxt(array_path) if array_path is not None else np.full(5, np.nan) for array_path in sublist] for sublist in array_paths_cold_higher ] ).reshape(*(array_paths_cold_higher.shape), len(temperatures), -1)
